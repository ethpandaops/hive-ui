...
export default function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (isDarkMode) {
      ...
      root.style.setProperty('--summary-bg', 'rgba(30, 41, 59, 0.7)');
      root.style.setProperty('--badge-bg', '#334155');
      root.style.setProperty('--table-header-bg', '#334155');
      root.style.setProperty('--stat-bg', '#334155');
      root.style.setProperty('--row-hover-bg', '#334155');

      // Status colors for dark theme
      root.style.setProperty('--success-bg', 'rgba(20, 83, 45, 0.5)');
      root.style.setProperty('--success-text', '#4ade80');
      root.style.setProperty('--success-border', '#22c55e');

      root.style.setProperty('--warning-bg', 'rgba(113, 63, 18, 0.5)');
      root.style.setProperty('--warning-text', '#fbbf24');
      root.style.setProperty('--warning-border', '#f59e0b');

      root.style.setProperty('--error-bg', 'rgba(127, 29, 29, 0.5)');
      ...
      root.style.setProperty('--summary-bg', 'rgba(248, 250, 252, 0.7)');
      root.style.setProperty('--badge-bg', '#f1f5f9');
      root.style.setProperty('--table-header-bg', '#f8fafc');
      root.style.setProperty('--stat-bg', '#f8fafc');
      root.style.setProperty('--row-hover-bg', '#f1f5f9');
